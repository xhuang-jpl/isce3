        ...
        for freq, pol_list, _ in get_cfg_freq_pols(self.cfg):
            ...
            self._create_2d_dataset(offset_group,
                                    ...
                                    description=("Digital Elevation Model (DEM) in radar coordinates."
                                                 ...
                                                 " to the WGS84 ellipsoid and projecting them onto a range/Doppler grid"
                                                 ),
                                    units=Units.meter)
            offset_group['digitalElevationModel'].attrs['disclaimer'] = np.bytes_('Disclaimer for DEM')
    ...
    def add_interferogram_to_swaths_group(self):
        ...
        for freq, pol_list, _ in get_cfg_freq_pols(self.cfg):
            ...
            self._create_2d_dataset(igram_group,
                                    ...
                                    units=Units.meter)
            igram_group['digitalElevationModel'].attrs['disclaimer'] = np.bytes_('Disclaimer for DEM')
